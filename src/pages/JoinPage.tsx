...
export default function JoinPage() {
  ...
  useEffect(() => {
    if (split && identity?.address) {
      const members = getSplitMembers(split.id);
      const existing = members.find((m) => m.walletAddress === identity.address);
  ...
  const handleJoin = async () => {
    ...
    try {
      // Validate this address can receive on testnet2
      let valid = true;
      if (client) {
        try {
          const resolved = await client.query('sphere_resolve', {
            identifier: identity.address,
          });
          valid = !!resolved;
        } catch {
          valid = true; // Allow if we can't validate
        }
      }
      ...
      if (split.distributionType === 'equal') {
        ...
        // For custom splits, join with 0 owed (creator will set amounts)
        amountOwed = 0n;
      }
      ...
      // Send payment request to this member if client available
  ...
  return (
    <div className="min-h-screen bg-black text-white flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        {/* Split card */}
        <motion.div
          className="rounded-3xl border border-orange-500/30 overflow-hidden mb-6"
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              ...
              {split.distributionType === 'equal' && (
                <div className="p-3 rounded-xl bg-orange-500/10 border border-orange-500/20">
                  ...
                  <div className="font-bold text-orange-400">
                    {formatTokenAmount(yourShare, token?.decimals ?? 18)} {split.tokenSymbol}
                  </div>
                </div>
              )}
            </div>
            ...
            {joined || alreadyMember ? (
              <div className="text-center">
                ...
                <p className="text-green-400 font-bold mb-4">
                  {joined ? "You've joined the split!" : "You're already in this split."}
                </p>
                <button
              ...
              <motion.button
                ...
                className="w-full py-4 rounded-2xl font-black text-black bg-gradient-to-r from-orange-400 to-orange-600 hover:from-orange-500 hover:to-orange-700 shadow-lg shadow-orange-500/30"
              ...
              <motion.button
                onClick={handleJoin}
                disabled={joining}
                className="w-full py-4 rounded-2xl font-black text-black bg-gradient-to-r from-orange-400 to-orange-600 hover:from-orange-500 hover:to-orange-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-orange-500/30"
